...
PropertiesService.getScriptProperties().setProperty('formCommsName', 'Tennis / Gymnastics Application');

// The name mask for sent emails
PropertiesService.getScriptProperties().setProperty('senderMask', 'Danebank Forms');

// The title of the column where confirmation emails should be sent to (eg the column with parent email addresses etc)
PropertiesService.getScriptProperties().setProperty('emailColumn', 'Email Address');
...
PropertiesService.getScriptProperties().setProperty('workflowType', 1);
...
function SendWorkflowMail(e,workflowStage) {
  ...
  try {
    ...
    var messageHTML,  textbody, cc = "", recipient = "";
    ...
    var initiatorEmail = e.namedValues[PropertiesService.getScriptProperties().getProperty('emailColumn')].toString(); // This is the submitter's email address
    var initiatorName;
    if(e.namedValues["First Name"].toString() == "" && e.namedValues["Surname"].toString() == "") { initiatorName = initiatorEmail; }
    else { initiatorName = e.namedValues["Salutation"].toString() + " " + e.namedValues["First Name"].toString() + " " + e.namedValues["Surname"].toString(); }
